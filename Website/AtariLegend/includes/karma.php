...
function KarmaGood()
{
	$sql = mysql_query("SELECT karma,userid,user_id FROM users WHERE permission=2 ORDER BY karma DESC LIMIT 15");
...
function KarmaBad()
{
	$sql = mysql_query("SELECT karma,userid,user_id FROM users WHERE karma IS NOT NULL AND permission=2 ORDER BY karma ASC LIMIT 15");
 	while($row=mysql_fetch_array($sql))$result[]=$row;
return $result;	
}
