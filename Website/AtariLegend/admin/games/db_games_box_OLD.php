...
for($i=1; $i <= 1; $i++)
{
	if($image[$i] != 'none')
	{
		...
		if ($mode == "back")
		{
			 ...
			 $file_data = rename("$file_data", "$game_boxscan_path$backbox[0].jpg");
		...
		else
		{
			...
			$box_id = $boxCover[0];
			 ...
			 // Rename the uploaded file to its autoincrement number and move it to its proper place.
			 $file_data = rename("$file_data", "$game_boxscan_path$boxCover[0].jpg");
...
unlink ("$game_boxscan_path$game_boxscan_id.png");
