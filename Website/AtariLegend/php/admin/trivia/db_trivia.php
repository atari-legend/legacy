...
if (isset($action) and $action == "did_you_know_insert") {
    ...
    $sql = $mysqli->query("INSERT INTO trivia (trivia_text) VALUES ('$trivia_text')")
        or die("Couldn't insert trivia text");
...
if (isset($action) and $action == "did_you_know_delete") {
    if ($_SESSION['permission']==1 or $_SESSION['permission']=='1') {
        ...
        $sql = $mysqli->query("DELETE FROM trivia WHERE trivia_id = '$trivia_id'")
            or die("Couldn't delete trivia text");
...
if (isset($action) and $action == "delete_trivia_quote") {
    if ($_SESSION['permission']==1 or $_SESSION['permission']=='1') {
        ...
        $sql = $mysqli->query("DELETE FROM trivia_quotes WHERE trivia_quote_id = '$trivia_quote_id'")
            or die("couldn't delete trivia quote");
...
if (isset($action) and $action == "add_trivia") {
    if (isset($trivia_quote)) {
        ...
        $mysqli->query("INSERT INTO trivia_quotes (trivia_quote) VALUES ('$trivia_quote')")
...
if (isset($action) and $action == "edit_trivia_quote") {
    if (isset($trivia_text)) {
        if ($_SESSION['permission']==1 or $_SESSION['permission']=='1') {
            ...
            $mysqli->query("UPDATE trivia_quotes SET trivia_quote='$trivia_quote'
                WHERE trivia_quote_id = $trivia_quote_id") or die('Error: ' . mysqli_error($mysqli));
...
if (isset($action) and $action == "update_trivia") {
    if (isset($trivia_text)) {
        if ($_SESSION['permission']==1 or $_SESSION['permission']=='1') {
            ...
            $mysqli->query("UPDATE trivia SET trivia_text='$trivia_text' WHERE trivia_id = $trivia_id")
                or die('Error: ' . mysqli_error($mysqli));
...
if ($action == "spotlight_delete") {
    if (isset($spotlight_id)) {
        if ($_SESSION['permission']==1 or $_SESSION['permission']=='1') {
            ...
            $query_spotlight_screenshot = $mysqli->query("SELECT * FROM spotlight
                WHERE spotlight_id = " . $spotlight_id . "")
                or die("something is wrong with mysqli of spotlight screenshots");
            ...
            $sql = $mysqli->query("DELETE FROM spotlight WHERE spotlight_id = '$spotlight_id'")
                or die("couldn't delete spotlight");
            ...
            foreach ($image['tmp_name'] as $key => $tmp_name) {
                if ($tmp_name !== 'none') {
                    ...
                    if ($ext !== "") {
                        ...
                        $sdbquery = $mysqli->query("INSERT INTO screenshot_main (screenshot_id,imgext)
                            VALUES ('','$ext')") or die("Database error - inserting screenshots");

                        $new_screenshot_id = $mysqli->insert_id;

                        $spot_text = $mysqli->real_escape_string($spot_text);

                        $mysqli->query("INSERT INTO spotlight ( spotlight, screenshot_id, link )
                            VALUES ('$spot_text', $new_screenshot_id, '$link')")
                            or die("Inserting the spotlight failed");
                        ...
                        $file_data = rename(
                            $image['tmp_name'][$key],
                            "$spotlight_screenshot_save_path$screenshotrow[0].$ext"
                        );
...
if (isset($action) and $action == "update_spotlight") {
    if (isset($spot_text)) {
        if ($_SESSION['permission']==1 or $_SESSION['permission']=='1') {
            ...
            $mysqli->query("UPDATE spotlight SET spotlight='$trivia_text' WHERE spotlight_id = $spotlight_id")
                or die('Error: ' . mysqli_error($mysqli));
