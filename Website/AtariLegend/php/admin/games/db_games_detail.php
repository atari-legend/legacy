...
if (isset($action) and $action == 'modify_game') {
    ...
    $sdbquery = $mysqli->query("DELETE FROM game_origin_cross WHERE game_id=$game_id");
    $sdbquery = $mysqli->query("DELETE FROM game_devtool_cross WHERE game_id=$game_id");

    // Insert the all the attributes again
    if (isset($software_origin)) {
        foreach ($software_origin as $line) {
            $sdbquery = $mysqli->query("INSERT INTO game_origin_cross (game_id,software_origin_id) VALUES ($game_id,$line)");
        }
    }
    if (isset($software_devtool)) {
        foreach ($software_devtool as $line) {
            $sdbquery = $mysqli->query("INSERT INTO game_devtool_cross (game_id,software_devtool_id) VALUES ($game_id,$line)");
        }
    }
