...
$sql_game = $mysqli->query("SELECT * FROM game
                       WHERE game_id='$game_id' ORDER BY game_name") or die("Error getting game info");
...
$sql_similar = $mysqli->query("SELECT * FROM game_similar
                            ...
                            WHERE game_similar.game_id='$game_id'") or die("Couldn't query similar games");
$nr_similar = 0;
while ($similar = $sql_similar->fetch_array(MYSQLI_BOTH)) {
    $smarty->append('similar', array(
        'game_similar_id' => $similar['game_similar_id'],
...
while ($game_similar = $sql_game_similar->fetch_array(MYSQLI_BOTH)) {
    $smarty->append('similar_games', array(
        'game_id' => $game_similar['game_id'],
