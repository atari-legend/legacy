...
if (isset($doc_category_id) and isset($action) and $action == 'update') {
    $sdbquery = $mysqli->query("UPDATE doc_category SET doc_category_name = '$doc_category_name'
        WHERE doc_category_id = $doc_category_id") or die("Couldn't Update the doc category");
...
if (isset($doc_category_id) and isset($action) and $action == 'delete_doc_category') {
    ...
    if ($sql->num_rows > 0) {
        ...
        $mysqli->query("DELETE FROM doc_category WHERE doc_category_id = $doc_category_id")
            or die("Failed to delete doc category");
...
if (isset($action) and $action == 'insert_category') {
    if ($category_name == '') {
        ...
        $sql_doccategory = $mysqli->query("INSERT INTO doc_category (doc_category_name) VALUES ('$category_name')")
            or die("error inserting doc category");
