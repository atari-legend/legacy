...
else
{
	...
	$sql_doc_category_edit = "SELECT * FROM doc_category WHERE doc_category_id = $doc_category_id";
	$result_doc_category_edit= $mysqli->query($sql_doc_category_edit) or die ("error getting doc category");

	while ( $row=$result_doc_category_edit->fetch_array(MYSQLI_BOTH) ) 
	{
		$smarty->assign('doc_category_edit',
		array('doc_category_id_edit' => $row['doc_category_id'],
