...
if (isset($mode) and $mode == 'review')
{
    $sql_review = $mysqli->query("SELECT * FROM game
                           ...
                           AND review_main.review_edit = '0'") or die("Error - Couldn't query review data");
    ...
    while ($sql_screenshots = $query_screenshots->fetch_array(MYSQLI_BOTH)) {
        ...
        $smarty->append('screenshots_review', array(
            'screenshot' => $new_path,
...
if (isset($mode) and $mode == 'interview')
{
    ...
    //$query_author = $mysqli->query("SELECT * FROM users WHERE user_id = '$query_interview[user_id]'") or die("couldn't get author of interview");
