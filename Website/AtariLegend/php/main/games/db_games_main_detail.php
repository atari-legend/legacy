    ...
    if ($action=='delete_comment') {
        ...
        if (isset($subtype) and $subtype == 'medium') {
            if ($textfield_medium == '' or  $textfield_medium == 'Game info') {
                ...
                $mysqli->query(
                    "INSERT INTO game_submitinfo (game_id, user_id, timestamp, submit_text, game_done ) "
                    ."VALUES ('$game_id', '$_SESSION[user_id]', '$timestamp', '$textfield_medium', '2')"
                )
                    or die("Inserting the game submission failed");
                        ...
                        if ($size_image < $maxsize) {
                            if ($ext !== "") {
                                ...
                                $SCREENSHOT = $mysqli->query(
                                    "SELECT screenshot_id FROM screenshot_main
                                ...
                                $sdbquery = $mysqli->query(
                                    "INSERT INTO screenshot_game_submitinfo "
                                    ."(game_submitinfo_id, screenshot_id) VALUES ($new_game_submit_id, $screenshot_id)"
                                )
                                ...
                                $file_data = rename(
                                    $image['tmp_name'][$key],
                                    "$game_submit_screenshot_save_path$screenshotrow[0].$ext"
                                );
            ...
            if ($textfield == '' or  $textfield == 'Game info') {
                ...
                $mysqli->query(
                    "INSERT INTO game_submitinfo (game_id, user_id, timestamp, submit_text, game_done ) "
                    ."VALUES ('$game_id', '$_SESSION[user_id]', '$timestamp', '$textfield', '2')"
                )
                    or die("Inserting the game submission failed");
        ...
        $mysqli->query("INSERT INTO game_user_comments (game_id, comment_id) VALUES ('$game_id', '$new_comment_id')")
            or die("Inserting the comment failed");
