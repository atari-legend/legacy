...
$sql_game_attributes = $mysqli->query("SELECT * FROM game_devtool_cross
                LEFT JOIN software_devtool ON (software_devtool.software_devtool_id = game_devtool_cross.software_devtool_id)
               WHERE game_id='$game_id'") or die('Error: ' . mysqli_error($mysqli));

while ($game_software_devtool = $sql_game_attributes->fetch_array(MYSQLI_BOTH)) {
    $smarty->append('game_software_devtool', array(
        'software_devtool_id' => $game_software_devtool['software_devtool_id'],
        'software_devtool_name' => $game_software_devtool['software_devtool_name']
    ));
}
$sql_game_attributes = $mysqli->query("SELECT * FROM game_origin_cross
                LEFT JOIN software_origin ON (software_origin.software_origin_id = game_origin_cross.software_origin_id)
               WHERE game_id='$game_id'") or die('Error: ' . mysqli_error($mysqli));

while ($game_software_origin = $sql_game_attributes->fetch_array(MYSQLI_BOTH)) {
    $smarty->append('game_software_origin', array(
        'software_origin_id' => $game_software_origin['software_origin_id'],
        'software_origin_name' => $game_software_origin['software_origin_name']
    ));
}
        ...
        while ($interview = $sql_interview->fetch_array(MYSQLI_BOTH)) {
            ...
            if ($v_ind_image != 'none') {
                $smarty->append('interviews', array(
                    'ind_id' => $game_author['game_author_id'],
...
$sql_similar = $mysqli->query("SELECT * FROM game_similar WHERE game_similar.game_id = '$game_id' ORDER BY RAND() LIMIT 1") or die("Error - Couldn't get similar game");
...
while ($query_similar = $sql_similar->fetch_array(MYSQLI_BOTH)) {
    ...
    $query_game_similar_data = $mysqli->query("SELECT * FROM game
                                                            ...
                                                            ORDER BY RAND() LIMIT 1") or die("Error - Couldn't get similar game data");
...
//*********************************************************************************************
$sql_vs = $mysqli->query("SELECT amiga_id, C64_id FROM game_vs WHERE atari_id = '$game_id'") or die("Error - Couldn't get the Lemon links");
