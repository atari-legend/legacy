...
//***********************************************************************************
$sql_game = $mysqli->query("SELECT * FROM game WHERE game_id='$game_id'") or die("Error getting game info");
...
$sql_game_attributes = $mysqli->query("SELECT * FROM game_attributes
                LEFT JOIN attribute_type ON (attribute_type.attribute_type_id = game_attributes.attribute_type_id)
...
while ($game_attributes = $sql_game_attributes->fetch_array(MYSQLI_BOTH)) {
    $smarty->append('game_attributes', array(
        ...
        'attribute_type_name' => $game_attributes['attribute_type_name']
    ));
}
